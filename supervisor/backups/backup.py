...
class Backup(CoreSysAttributes):
    ...
    async def store_addons(self, addon_list: list[str]):
        ...
        async def _addon_save(addon: Addon):
            ...
            addon_file = SecureTarFile(
                Path(self._tmp.name, tar_name), "w", key=self._key, redacted_marker=None
            )
    ...
    async def restore_addons(self, addon_list: list[str]):
        ...
        async def _addon_restore(addon_slug: str):
            ...
            addon_file = SecureTarFile(
                Path(self._tmp.name, tar_name), "r", key=self._key, redacted_marker=None
            )
    ...
    async def store_folders(self, folder_list: list[str]):
        ...
        def _folder_save(name: str):
            ...
            try:
                ...
                with SecureTarFile(
                    tar_name, "w", key=self._key, redacted_marker=None
                ) as tar_file:
                    atomic_contents_add(
                        tar_file,
                        origin_dir,
        ...
        async def _folder_restore(name: str) -> None:
            ...
            def _restore() -> None:
                try:
                    ...
                    with SecureTarFile(
                        tar_name, "r", key=self._key, redacted_marker=None
                    ) as tar_file:
